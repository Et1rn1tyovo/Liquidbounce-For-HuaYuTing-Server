...
open class Module @JvmOverloads constructor(
    ...
    var state = false
        set(value) {
            ...
            if (!isStarting) {
                mc.soundHandler.playSound(PositionedSoundRecord.create(ResourceLocation("random.click"), if (value) 1F else 0.8F))
                addNotification(Notification("Module",translation("notification.module" + if (value) "Enabled" else "Disabled", getName()),if (value) NotifyType.SUCCESS else NotifyType.ERROR))
            }
