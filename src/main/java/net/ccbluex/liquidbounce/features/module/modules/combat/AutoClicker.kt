...
object AutoClicker : Module("AutoClicker", ModuleCategory.COMBAT) {
    private val maxCPSValue: FloatValue = object : FloatValue("MaxCPS", 8f, 1f..20f) {
        override fun onChange(oldValue: Float, newValue: Float) = newValue.coerceAtLeast(minCPS)
    }
    private val maxCPS by maxCPSValue

    private val minCPS by object : FloatValue("MinCPS", 5f, 1f..20f) {
        override fun onChange(oldValue: Float, newValue: Float) = newValue.coerceAtMost(maxCPS)
