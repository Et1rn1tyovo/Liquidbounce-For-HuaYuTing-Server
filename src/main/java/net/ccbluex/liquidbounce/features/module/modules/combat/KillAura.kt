...
object KillAura : Module("KillAura", ModuleCategory.COMBAT, Keyboard.KEY_R) {
    ...
    private var range: Float by object : FloatValue("Range", 3.7f, 1f..8f) {
    ...
    // AutoBlock
    private val autoBlock by ListValue("AutoBlock", arrayOf("Off", "Packet", "AfterTick", "Fake","KeyBind"), "Packet")
        ...
        if (event.eventState == EventState.POST) {
            ...
            if (canBlock) {
                when (autoBlock) {
                    ...
                    "KeyBind" -> startBlocking(currentTarget!!, hitable)
                }
            }
    ...
    fun onTick(event: TickEvent){
        if (target!!.isDead){
            Kills++
            LiquidBounce.eventManager.callEvent(EntityDeathEvent(target))
        }
    }
    /**
     * Update event
