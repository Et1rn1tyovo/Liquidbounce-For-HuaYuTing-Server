...
object AutoAccount : Module("AutoAccount", ModuleCategory.MISC) {
    ...
    private fun respond(msg: String) = when {
        register && "/reg" in msg -> {
            addNotification(Notification("Auto Account","Trying to register.",NotifyType.INFO))
        ...
        login && "/log" in msg -> {
            addNotification(Notification("Auto Account","Trying to log in.",NotifyType.INFO))
            Timer().schedule(sendDelay.toLong()) {
    ...
    private fun success() {
        if (status == Status.SENT_COMMAND) {
            addNotification(Notification("Auto Account","Logged in as ${mc.session.username}",NotifyType.INFO))
    ...
    private fun fail() {
        if (status == Status.SENT_COMMAND) {
            addNotification(Notification("Auto Account","Failed to log in as ${mc.session.username}",NotifyType.INFO))
    ...
    private fun changeAccount() {
        ...
        if (saveValue.isActive() && !accountsConfig.accountExists(account)) {
            ...
            addNotification(Notification("Auto Account","Saved alt ${account.name}",NotifyType.INFO))
        }
    }
