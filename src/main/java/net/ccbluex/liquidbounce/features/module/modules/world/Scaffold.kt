...
object Scaffold : Module("Scaffold", ModuleCategory.WORLD, Keyboard.KEY_I) {
    ...
    private val rotation_mode by ListValue("Rotation", arrayOf("Normal", "Backward"), "Normal")
    // Expand
    ...
    private fun search(blockPosition: BlockPos, raycast: Boolean, area: Boolean): Boolean {
        ...
        if (rotations) {
            ...
            val backwardrota = Rotation(180f,82f)

            if (rotation_mode.equals("Normal")){
                setRotation(limitedRotation, keepTicks)
            }else{
                setRotation(backwardrota, keepTicks)
            }
        }
