...
public class KawaseBlur{
    ...
    private static void initFramebuffers(float iterations) {
        ...
        for (int i = 1; i <= iterations; i++) {
            Framebuffer currentBuffer = new Framebuffer((int) (BloomUtils.mc.displayWidth / Math.pow(2, i)), (int) (BloomUtils.mc.displayHeight / Math.pow(2, i)), false);
    ...
    public static void renderBlur(int stencilFrameBufferTexture, int iterations, int offset) {
        if (currentIterations != iterations || framebuffer.framebufferWidth != BloomUtils.mc.displayWidth || framebuffer.framebufferHeight != BloomUtils.mc.displayHeight) {
            initFramebuffers(iterations);
            currentIterations = iterations;
        }

        renderFBO(framebufferList.get(1), BloomUtils.mc.getFramebuffer().framebufferTexture, kawaseDown, offset);
        ...
        BloomUtils.mc.getFramebuffer().bindFramebuffer(true);
