...
public class ShaderUtils {
    ...
    public ShaderUtils(String fragmentShadersrc, boolean notUsed) {
        ...
        try {
            vertexShaderID = createShader(Minecraft.getMinecraft().getResourceManager().getResource(new ResourceLocation("liquidbounce/shader/vertex.vsh")).getInputStream(), GL_VERTEX_SHADER);
        } catch (IOException e) {
    ...
    public static String getShaderResource(final String resource) {
        try {
            final InputStream inputStream = RESOURCE_MANAGER.getResource(new ResourceLocation("shaders/" + resource)).getInputStream();
    ...
    public static int createShader(InputStream inputStream, int shaderType) {
